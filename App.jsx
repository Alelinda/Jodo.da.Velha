...
const quadroInicial = Array(9).fill(''); //fill= preenchimento
...
const proximoJogador = jogadorAtual ==='X'? 'O': 'X'; //internário( IF(?) & ELSE(:) de forma resumida) alterna o jogador
...
useEffect(() => {
  ...
  } else if (quadro.every(celula => celula !== '')){ //every(celula) verifica se todas as celulas estão preenchidas e não há um vencedor, enyão empatou
    setEmpate(true);
  } else {
    setEmpate(false);
  }
...
const reiniciar = () => {
  setQuadro(quadroInicial); //reinicia o tabuleiro p estado inicial
  setJogadorAtual('X'); //começa jogador atual com x 
